...
function pgidOf(pid) {
  try {
    if (process.platform === 'darwin') return parsePid(execFileSync('ps', ['-o', 'pgid=', '-p', String(pid)], {encoding:'utf8'}));
...
function cmdlineOf(pid) {
  try {
    if (process.platform === 'darwin') return execFileSync('ps', ['-o', 'command=', '-p', String(pid)], {encoding:'utf8'});
...
function portOwners() {
  if (process.platform === 'darwin') {
    try {
      const output=execFileSync('lsof',['-nP','-t',`-iTCP:${PREVIEW_PORT}`,'-sTCP:LISTEN'],{encoding:'utf8'});
      return {pids:output.trim().split(/\s+/).map(parsePid).filter(Boolean),unattributed:false};
    } catch (error) {
      if (error.status===1) return {pids:[],unattributed:false};
      throw error;
    }
  }
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  ...
  if (!existsSync("/proc/self") && process.platform !== 'darwin') {
