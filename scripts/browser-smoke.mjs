...
try {
  ...
  for (const vp of VIEWPORTS) {
    ...
    const hasCanvas = (await Promise.all(frames.map(frame => frame.locator("canvas").count()))).some(Boolean);
    const canvasResolution = (await Promise.all(frames.map(frame => frame.evaluate(() => {
      const canvas = document.querySelector('canvas#c');
      if (!canvas) return null;
      const scale = Math.min(devicePixelRatio, innerWidth < 520 ? 1.4 : 2);
      return { actual: [canvas.width, canvas.height], expected: [innerWidth * scale | 0, innerHeight * scale | 0] };
    })))).filter(Boolean);
    for (const size of canvasResolution) if (String(size.actual) !== String(size.expected)) {
      errors.pageErrors.push(`Game canvas resolution ${size.actual} differs from viewport ${size.expected}`);
    }
    const bodyText = (await Promise.all(frames.map(frame => frame.locator('body').innerText().catch(() => '')))).join(' ');
  ...
  const brandWarnings = computeBrandWarnings({ workspaceRoot, hasCanvas: viewports.desktop.hasCanvas });
